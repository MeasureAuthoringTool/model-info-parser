...
export default function distinctDataTypes(input: Array<MemberVariable>, baseDataType: DataType|null, name: string, namespace: string): Array<DataType> {
  ...
  const allDataTypes: Array<DataType> = input.reduce((accumulator, currentMemberVar) => {
    accumulator.push(currentMemberVar.dataType);
    return accumulator;
  }, []);

  const result = allDataTypes.reduce((accumulator, currentType) => {
    if (!currentType.systemType && !typeMatch(currentType, name, namespace) && !_.find(accumulator, currentType)) {
  ...
  if (name !== 'Extension' && containsPrimitive(allDataTypes) && !result.find(type => type.typeName === 'Extension')) {
